...
## Task 060：修复悬浮面板长文本不可内部滚动

### 目标

修复截图翻译和剪贴板悬浮面板中“识别文字 / 原文”和“译文”内容过长时被固定行数截断的问题，让用户可以在文本卡片内部滚动查看完整内容。

### 范围

- 参考用户实测截图定位悬浮面板文本截断问题。
- 为截图翻译面板的“识别文字”和“译文”卡片增加内部滚动。
- 为剪贴板悬浮翻译面板的“原文”和“译文”卡片同步增加内部滚动。
- 保持状态文案和底部操作按钮固定可见。
- 运行编译和可用测试验证。
...
- 不改 OCR、翻译和朗读业务逻辑。
- 不改截图框选流程。
- 不发布新版内置更新包，除非后续确认需要发版。

### 完成标准

- 长识别文字可以在“识别文字”卡片内部上下滚动查看。
- 长译文可以在“译文”卡片内部上下滚动查看。
- 悬浮面板底部“重新框选 / 朗读译文 / 复制译文”等按钮仍固定可见。
- Debug 构建通过。

### 验证记录

- 已使用 imagegen 生成内部滚动版悬浮面板设计参考图：`docs/ui/floating-panel-scrollable-text-design.png`。
- 待代码修复完成后补充。

## Task 061：按当前项目润色与扩充课程设计论文

### 目标

对照仓库真实代码，对课程设计论文 `D:\code\app开发\app程序设计-唐国荣.docx` 做技术准确性纠正、内容适度扩充、文字润色与格式规范，作为期末提交稿。该文档由 `scripts/build_course_paper.py` 生成，本次通过修改脚本并重新运行实现（脚本自带"生成前自动备份原 docx"）。

### 范围

- 纠正与代码不符之处：架构图误写的 `AuthViewModel`、接口表 R2 分片路径与 `/history` 请求字段、ER 图云端表名。
- 补全真实但漏写的功能：悬浮球截图选区翻译（MediaProjection 抓帧→框选→OCR→翻译）、离线词典查询。
- 补精确参数：云端/HY-MT 生成参数、6 分片字节与 SHA256、PBKDF2/JWT/验证码策略、5 项权限、关键依赖版本。
- 适度扩充第五章（新增 5.6 截图翻译实现、5.7 跨应用入口与剪贴板策略，含参数表与少量关键代码），扩充第六章测试覆盖表，补第二/三章。
- 新增"截图翻译活动图"（脚本 PIL 绘制，风格与其余活动图一致）。
- 参考文献扩充并规范为 GB/T 7714 风格。

### 不包含

- 不改动 Android / Cloudflare Worker 业务代码。
- 不重写封面与中英文摘要的整体结构（仅小幅润色）。
- 不替换既有 9 张工程图的整体风格。

### 完成标准

- `py scripts/build_course_paper.py` 运行成功，生成新 docx，并自动产生 `_补全前备份_时间戳.docx`。
- 新 docx 含截图翻译流程与配图、第五章新增小节、参数表、GB/T 7714 参考文献。
- 重新抽取新 docx 校验：标题层级、图表题注与编号连续、关键技术数值与代码一致。
- 篇幅约 28-30 页。

### 验证记录

- 已在 Task 062 最终润色中继续完成并验证：`scripts/build_course_paper.py` 可正常生成正式 Word，文档已自动备份、重新生成并更新目录。
- 正式文档已包含截图翻译流程图、第五章新增实现小节、参数表、关键代码块、测试覆盖表和规范化参考文献。
- 最终抽取校验见 Task 062。

## Task 062：课程设计论文最终润色与真机截图补强

### 目标

在已生成课程设计论文的基础上，按“稳妥课程设计风格 + 适度突出项目亮点”继续优化最终稿。重点补齐封面成员信息、真实 App 运行截图、测试章证据链、项目特色说明，并重新生成 Word 目录，确保文档可直接提交。

### 范围

- 补全封面成员信息：
  - 组长：唐国荣，学号：202302150016。
  - 成员：王俊杰，学号：202302150022。
  - 成员：袁志城，学号：202302150045。
- 通过已连接 Android 手机获取真实运行截图，并保存到论文截图目录。
- 在第五章加入真实运行截图与说明，覆盖首页翻译、模型服务/设置、历史记录、悬浮球菜单、截图框选与截图翻译结果等关键流程。
- 扩写第六章测试内容，补充构建命令、自动化测试、后端安全测试、真机验证和异常场景说明。
- 增加或强化“系统特色与创新点”表述，集中体现云端/离线混合翻译、跨应用入口、悬浮球截图翻译、R2 分片下载和轻量云端后端。
- 检查“离线词典”等表述与当前项目一致，避免摘要、总结和正文之间出现功能悬空。
- 重新生成正式 Word，并使用 Microsoft Word 自动更新目录域。

### 不包含

- 不修改 Android / Cloudflare Worker 业务代码。
- 不发布新的 APK 或 R2 更新包。
- 不删除历史备份文档和干跑产物。

### 完成标准

- 正式文档 `D:\code\app开发\app程序设计-唐国荣.docx` 更新成功，并在覆盖前产生备份。
- 封面成员信息完整。
- 文档包含真实 App 截图，且截图说明与当前项目功能一致。
- 第六章测试说明比原稿更充分，包含自动化测试、构建验证、后端测试、真机验证和异常场景。
- 目录可见、页码正常、无 Word 错误域。
- 抽取 docx 验证段落、表格、图片、目录跳转和新增关键词均通过。

### 验证记录

- 已通过 ADB 从已连接 Android 真机获取论文截图，保存于 `docs/paper_screenshots/`。
- 已修改 `scripts/build_course_paper.py`，补全封面成员信息、系统特色与创新点、第五章真机截图、5.8 离线词典与历史管理、第六章真机验证与异常场景。
- `py` AST 语法检查通过。
- 干跑生成成功：`app程序设计-唐国荣_FINAL_DRYRUN_20260530_232310.docx`，段落 197、表格 18、图片 19。
- 正式生成成功：`D:\code\app开发\app程序设计-唐国荣.docx`。
- 自动备份成功：`D:\code\app开发\app程序设计-唐国荣_补全前备份_20260530_232348.docx`。
- 已使用 Microsoft Word COM 自动更新目录域：目录对象 1 个，更新域 87 个。
- 抽取校验通过：文件大小 3,924,864 字节，媒体文件 19，`PAGEREF = 43`，目录锚点 129，Word 错误域 0。
- `python-docx` 校验通过：段落 240、表格 18、内嵌图片 19；7 张真机截图图题全部存在。
- Word 级别读取通过：37 页、目录对象 1 个、域 87 个、段落 814、表格 18、图片 19。

## Task 063：单独生成课程系统设计文档

### 目标

基于课程论文正式稿 `D:\code\app开发\app程序设计-唐国荣.docx` 和当前项目真实实现，完成老师要求单独提交的系统设计文档 `D:\code\app开发\学习资料\课程论文_《APP开发课程设计》系统设计论文.docx`。目标文档按原模板栏目组织，主要复用正式稿第四章“系统设计”的文字、图表和表格。

### 范围

- 保留目标文档的课程系统设计论文定位。
- 按模板顺序整理为：业务流程设计、接口设计、类设计、数据库设计、系统架构设计。
- 复用正式稿中的注册登录、文本翻译、图片 OCR、模型下载与更新、悬浮球截图翻译流程说明和活动图。
- 复用正式稿中的接口说明、核心类图、数据库 ER 图和物理表结构说明。
- 生成前备份目标 Word 文件。
- 结合 Word/OOXML 抽取结果进行结构校验，并尽力进行渲染或视觉检查。
...
- 目标文档成功写入完整系统设计内容。
- 文档包含业务流程、接口、类、数据库、系统架构五类内容。
- 文档包含系统设计关键图表，图片数量和核心章节文本可抽取验证。
- 生成前自动保留目标文档备份。
- 验证结果写回本任务文档和 TODO 日志。

### 验证记录

- 已生成目标文档：`D:\code\app开发\学习资料\课程论文_《APP开发课程设计》系统设计论文.docx`。
- 生成前已保留原始模板备份：`课程论文_《APP开发课程设计》系统设计论文_备份_20260603_182102.docx`。
- 结构抽取校验通过：段落 59、表格 6、正文内嵌图片 10。
- 封面信息校验通过：题目为“基于 Android 的 AI 翻译 App 系统设计”，团队成员为唐国荣、王俊杰、袁志城。
- 章节关键字校验通过：业务流程设计、接口设计、类设计、数据库设计、系统架构设计均存在。
- 关键内容校验通过：截图翻译业务活动图、TranslationRepository、translation_history 等系统设计要点均可抽取。
- 视觉验证说明：Microsoft Word 后台导出 PDF 曾成功生成 13 页预览并检查封面和正文图表页；后续导出命令在 Word COM 后台偶发卡住，已停止残留进程，最终以结构校验和已导出预览结果作为交付依据。

## Task 064：完善关于与系统更新详情页和供应商配置滚动

### 目标

补全设置页“关于与系统更新”中的“开发许可协议”和“安全与隐私保护”内容，并采用设置列表 / 层级导航 / 推入式详情页的交互方式呈现。同时修复“供应商配置”底部弹窗在 MuMu 等模拟器小窗口下底部内容被遮挡、内部滚动不顺的问题。

### 范围

- 为“开发许可协议”增加可点击设置项，推入详情页展示第三方依赖、模型许可、云端接口责任和使用约束。
- 为“安全与隐私保护”增加可点击设置项，推入详情页展示本地存储、账号同步、网络请求、悬浮窗 / 截图权限和更新包校验说明。
- “关于与系统更新”页保持应用版本与更新检测能力不变。
- “供应商配置”底部弹窗增加高度约束、内部滚动和键盘避让，确保底部输入框在模拟器中可达。
- 生成 UI 设计图并保存到 `docs/ui/`。

### 不包含

- 不修改 Cloudflare Worker 后端接口。
- 不修改 R2 更新清单协议。
- 不发布新的 R2 更新包。

### 完成标准

- `docs/ui/` 下存在本次设置详情页设计图。
- “开发许可协议”和“安全与隐私保护”均从“关于与系统更新”页点击进入详情页，可返回上一级。
- 两个详情页内容完整、分组清晰，使用设置列表样式而非大段堆叠文本。
- “供应商配置”弹窗内容可内部滚动，键盘弹出时输入框不被遮挡。
- Debug 构建通过，并在模拟器中完成页面打开和截图验证。

### 验证记录

- 已使用 imagegen 生成设置详情页设计图：`docs/ui/about-license-privacy-settings-list-design.png`。
- 已在“关于与系统更新”中将“开发许可协议”和“安全与隐私保护”改为可点击设置项，点击后以推入式导航进入详情页。
- “开发许可协议”详情页已补充课程项目使用、用户配置接口、Hy-MT 离线模型、Google ML Kit、开源依赖和禁止行为说明。
- “安全与隐私保护”详情页已补充本地优先、账号同步、云端翻译请求、悬浮窗和截图、更新包校验、数据清理和最小化收集说明。
- 已修复“供应商配置”底部弹窗：直接全展开、内部可滚动、保留键盘避让，并补充输入框被键盘遮挡时的滑动提示。
...
## Task 065：发布 1.1.2 设置详情与供应商配置修复版本

### 目标

将“关于与系统更新”详情页补全和“供应商配置”滚动修复发布为 `1.1.2 (13)` Debug 内置更新包，并推送到 GitHub。用户本次需要自行通过内置更新测试，因此不安装到模拟器。

### 范围

- 将 App 默认版本提升为 `1.1.2 (13)`。
- 更新 R2 Debug 发布脚本默认版本、APK 对象路径和更新说明。
- 构建 `1.1.2` Debug APK。
- 上传 APK 到 R2，并更新 `releases/latest.json`。
- 校验公开 `latest.json` 与 APK URL 可访问。
- 将本次功能、版本和发布记录提交并推送到 GitHub。
...
- `app/build.gradle.kts` 默认版本为 `1.1.2 (13)`。
- `scripts/publish-r2-debug-update.ps1` 默认发布 `1.1.2` APK。
- Debug APK 构建成功。
- R2 `releases/latest.json` 指向 `1.1.2 (13)`，APK URL 可公开访问。
- GitHub 已收到本次提交。
