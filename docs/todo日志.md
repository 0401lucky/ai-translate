...
## 2026-05-27：完善 APP 开发课程设计论文

- [x] 明确完成标准：主文档内容按评分标准补全，覆盖需求分析、系统设计、实现与测试，图表清晰且与当前 AI 翻译 App 实际一致。
- [x] 读取主文档、系统需求分析评分标准、系统设计评分标准和绘图要求。
- [x] 梳理当前项目功能、架构、数据库和部署现状。
- [x] 生成并插入必要图示：功能模块图、用例图、流程图/活动图、类图、ER 图和系统架构图。
- [x] 写回 `app程序设计-唐国荣.docx` 并保留备份。
- [x] 渲染检查 Word 排版，修正图表遮挡、分页和文字溢出问题。

## 2026-05-27：按学校封面和优秀论文格式修正文档排版

- [x] 明确完成标准：恢复原封面学校图片与封面版式，正文标题字号和颜色贴近优秀毕业设计参考文档，图表清晰且无明显排版问题。
- [x] 读取 `封面_补全前备份_20260522.docx` 和优秀毕业设计参考文档的页面、样式、图片信息。
- [x] 恢复主文档封面图片和封面横线填写式样。
- [x] 将正文标题、图题、表格和页码调整为论文模板风格，去掉不合适的蓝色标题。
- [x] 导出 PDF 预览并逐页检查。
- [x] 根据用户复查反馈重新修正封面：恢复两行小组成员姓名/学号，移除误用的指导教师、企业导师、二级学院和封面日期字段。
- [x] 重新生成 Word 文档，使用 Word 自身目录功能生成目录并导出 20 页 PDF 预览。
- [x] 抽查封面、目录页和核心图表页，确认封面字段已恢复，正文标题无蓝色残留，图表未发现明显遮挡。
- [x] 复查封面成员行，修正“学号”横线换行问题。
- [x] 根据参考论文补充摘要前中文题目、英文题目和英文摘要页。
- [x] 重新生成 Word 和 PDF 预览，确认中文摘要页、英文摘要页和目录页顺序正确。
- [x] 根据绘图要点 PDF 返工第四章活动图，补充起点、终点、判断分支、条件标注和回退路径。
- [x] 替换图 4-2 至图 4-5 为 UML 活动图风格，并重新生成 Word 与 PDF 预览。
- [x] 抽查第 12 至 15 页活动图，确认图题、图形和正文分页正常。
- [x] 根据用户复查反馈，将图 4-2、图 4-3、图 4-5 中容易产生误解的红色粗横线改为小菱形汇合节点。
- [x] 修正图 4-3 普通连线穿过“模型可用？”判断文字的问题，重新生成主文档和 22 页 PDF 预览。
- [x] 抽查 PDF 目录页和第 12 至 15 页活动图，确认目录占位符消失、图题正常、活动图无明显遮挡。

## 2026-05-27：应用图标与悬浮球重设计、剪贴板误触发修复

- [x] 明确完成标准：新图标和悬浮球设计稿落盘，Android 资源替换，拍照/相册返回不再触发旧剪贴板快捷翻译，测试和构建通过。
- [x] 使用 imagegen 生成应用图标设计稿，保存到 `docs/ui/app-icon-floating-redesign.png`。
- [x] 使用 imagegen 生成悬浮球设计稿，保存到 `docs/ui/floating-bubble-redesign.png`。
- [x] 更新 launcher icon 与悬浮球实现。
- [x] 修复剪贴板快捷翻译触发策略：限制最近 2 分钟复制、图片翻译中拦截、拍照/相册返回跳过一次提示。
- [x] 相关单元测试通过：`ClipboardQuickTranslatePolicyTest`、`TranslateViewModelTest`。
- [x] Debug APK 构建通过：`.\gradlew.bat :app:assembleDebug --no-daemon --max-workers=1 --console=plain`。
- [x] `git diff --check` 通过，仅有 Windows 换行提示。
- [x] 回写验证结果到 Task 文档和 TODO 日志。
- [ ] 当前无在线设备，真机拍照/相册返回链路待设备连接后补充点击验证。

## 2026-05-27：悬浮球截图框选翻译功能规划

- [x] 明确完成标准：规划悬浮球截图 / 框选翻译实施路线，并把生词本 / 句子收藏复习放入后续功能池。
- [x] 查询 Android 官方文档，确认 `MediaProjection` 授权、前台服务类型和 Android 14+ 会话限制。
- [x] 更新 `docs/悬浮窗划词翻译规划.md`，补充用户流程、MVP、技术方案、权限风险、分阶段落地和完成标准。
- [x] 将“生词本 / 句子收藏与复习”加入后续功能池，暂不进入当前实现。
- [x] 更新 `docs/task文档.md`，新增 Task 051 并写入验证记录。
- [x] 本轮仅做规划文档更新，未运行 Android 构建。

## 2026-05-27：实现悬浮球截图框选翻译 MVP

- [x] 明确完成标准：悬浮球可进入截图翻译，授权后框选屏幕区域，OCR 后自动翻译并在悬浮卡片展示结果。
- [x] 更新 `docs/task文档.md`，新增 Task 052。
- [x] 使用 imagegen 生成截图框选翻译 UI 设计图并保存到 `docs/ui/floating-screenshot-translate-design.png`。
- [x] 新增 MediaProjection 授权 Activity。
- [x] 新增一次性截图捕获服务和 Android 14+ 所需权限 / 前台服务类型声明。
- [x] 扩展 OCR 支持 Bitmap 输入。
- [x] 改造悬浮球菜单，提供剪贴板翻译和截图翻译两个入口。
- [x] 实现框选层、确认 / 取消 / 重新框选。
- [x] 接入 OCR + 翻译 + 悬浮结果卡片。
- [x] 增加可测试逻辑单元测试。
- [x] 运行定向单元测试和 Debug APK 构建。
- [x] `git diff --check` 通过，仅有 Windows 换行提示。
- [ ] 当前无在线设备，真机悬浮球截图链路待设备连接后补充点击验证。
- [x] 回写验证结果到 Task 文档和 TODO 日志。

## 2026-05-28：发布 1.0.9 图标悬浮截图翻译版本

- [x] 明确完成标准：版本号提升为 `1.0.9 (10)`，Debug APK 构建成功，R2 `latest.json` 指向新版，公开 URL 可访问，并推送 GitHub。
- [x] 更新 `docs/task文档.md`，新增 Task 053。
- [x] 将 App 默认版本号提升为 `1.0.9 (10)`。
